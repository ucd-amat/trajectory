...
def takeoff(V_rot,theta_climb_deg,rotation_rate_degps,gtow,RPM = 10000):
    ...
    while x[0] < 1000: #ft
        ...
        dx = np.array([
            x[1], #x_dot
            (Thrust*np.cos(theta)-D*np.cos(alpha)-L*np.sin(alpha))/(gtow/32.2),
            x[3], #y_dot
            (Thrust*np.sin(theta)-D*np.sin(alpha)+L*np.cos(alpha))/(gtow/32.2) - 32.2,
        ])
...
def landing(V_init,theta_glide,alt_init,gtow,RPM = 10000):
    x = np.zeros(4) #initialize x_dot
    x[1] = V_init
    x[2] = alt_init
    mu = 0.03
    t = 0
    ...
    # while x[2] < 35 and x[0] < 1000: #ft
    while x[0] < 1000 and x[1] >= 0: #ft
        t += dt

        normal_airspeed_fps = ((x[1]*np.cos(theta_glide))**2 + (x[3]*np.sin(theta_glide))**2)**(0.5)
        airspeed_fps = ((x[1])**2 + (x[3])**2)**(0.5)

        prop_data = prop_performance(RPM,normal_airspeed_fps)
        Thrust = prop_data["Thrust_lbf"]*0
        Power = prop_data["PWR_W"]*0
        qS = ((1/2)*rho*airspeed_fps**2*S)
        alpha = theta - np.nan_to_num(np.arctan(x[3]/x[1]))
        CL = cl_max*np.sin(2*np.pi*(alpha + theta0)/cl_max) #assume linear lift slope
        CD = dragCoeff_estimate(CL)
        D = CD*qS
        L = CL*qS
        ...
        dx = np.array([
            x[1], #x_dot
            (Thrust*np.cos(theta)-D*np.cos(alpha)-L*np.sin(alpha))/(gtow/32.2), # +x force
            x[3], #y_dot
            (Thrust*np.sin(theta)-D*np.sin(alpha)+L*np.cos(alpha))/(alpha-gtow/32.2) - 32.2, # +y force
        ])
        ...
        history["u"].append(x[1])
        history["v"].append(x[3])
        history["theta"].append(theta_glide)
        history["alpha"].append(alpha)
        history["CL"].append(CL)
        history["CD"].append(CD)
        history["Power"].append(Power)
...
plt.plot([0,history["x"][-1]],[1.4,1.4],label="C_L max") # hypothetical CL_max = 1.4 straight line
